...
**AI mirror** (`src/aiMirror.ts`): because `accessdb:` documents are virtual (no real file on disk), external tools like Claude Code's file/Read/Edit tools can't see them — and, just as importantly, can't see a *selection* made in one either. `AiMirrorManager` mirrors every object to real files under `<db-folder>/.accdb-ai/<Category>/<name><ext>`, keeps them in sync both ways (Access change → mirror file rewritten; external edit to the mirror file → pushed back into Access through the *same* `saveObject` pipeline as Ctrl+S, then offers a revert), and tears the mirror down when the database closes (it's a derived artifact, never a durable store — though the files themselves persist on disk as a cross-session cache, see `primeFromDisk`). This is the reason a Claude Code session can read/edit `.bas`/`.form.txt` files that look like normal files but are actually backed by a live Access connection. **CRITICAL AI EDITING RULE:** Because saving to Access via COM is a slow, serial process (conflict check → backup → write → compile), AI agents MUST batch all edits to a single file into a single tool call (e.g. using `multi_replace_file_content`). DO NOT make multiple sequential edits to the same mirrored file, as rapid file system events will cause COM conflicts and cause subsequent edits to be dropped. Always verify the change succeeded by re-reading the file after editing.
