...
- **`skills/`** — AI Agent skills (Romanian-language SOPs) for working with the VBA code *inside* Access projects opened through this extension (adding error handling, analyzing routines, fixing logged runtime errors). These are documentation/instructions, not application code.
...
**AI mirror** (`src/aiMirror.ts`): because `accessdb:` documents are virtual (no real file on disk), external AI tools can't see them — and, just as importantly, can't see a *selection* made in one either. `AiMirrorManager` mirrors every object to real files under `<db-folder>/.accdb-ai/<Category>/<name><ext>`, keeps them in sync both ways (Access change → mirror file rewritten; external edit to the mirror file → pushed back into Access through the *same* `saveObject` pipeline as Ctrl+S, then offers a revert), and tears the mirror down when the database closes (it's a derived artifact, never a durable store — though the files themselves persist on disk as a cross-session cache, see `primeFromDisk`). Through `src/mirrorDependencies.ts`, the mirror automatically resolves and extracts dependencies (SQL tables/queries, DLookup domains, and called VBA procedures) so that AI tools get a broader, more complete context of related files automatically. This is the reason an AI Agent session can read/edit `.bas`/`.form.txt` files that look like normal files but are actually backed by a live Access connection. **CRITICAL AI EDITING RULE:** Because saving to Access via COM is a slow, serial process (conflict check → backup → write → compile), AI agents MUST batch all edits to a single file into a single tool call (e.g. using `multi_replace_file_content`). DO NOT make multiple sequential edits to the same mirrored file, as rapid file system events will cause COM conflicts and cause subsequent edits to be dropped. Always verify the change succeeded by re-reading the file after editing.

**Session persistence** (`src/extension.ts`): every open database is remembered by path in `context.globalState` (written in `openDatabase`, forgotten on an explicit `closeDatabase`) and transparently reopened — bridge, tree, and AI-mirror watcher included — the next time the extension activates (`reopenPersistedDatabases`, fire-and-forget, sequential, gated by the `accessExplorer.reopenOnStartup` setting). This matters because *all* of the above (registry, mirror watchers, `AccessFsProvider`'s per-object cache) is otherwise pure in-memory state that a "Developer: Reload Window" wipes with zero warning — before this existed, an AI edit to an already-mirrored file after a reload would silently go nowhere (no watcher left to observe it, no error, nothing). `AccessFsProvider.refreshOpenDocuments` additionally re-fetches and fires a change event for any of that database's `accessdb:` tabs VS Code already had open, so a clean (non-dirty) restored tab shows live content again without a manual close/reopen.

**Other notable pieces:**
- `src/aiContext.ts` + `src/vbaSymbols.ts` — "Select Code for AI" (`Ctrl+Alt+A`): groups Sub/Function/Property procedures the way the VBA IDE's dropdowns do (`cmdSave_Click` → bucket `cmdSave`, event `Click`; unmatched → `(General)`) and applies the chosen scope as a real selection in the object's **AI-mirror file** (opening/focusing it if needed), not in the `accessdb:` editor — since neither a virtual document nor a bare cursor position (e.g. from the native breadcrumbs/outline combo, which only moves the cursor) ever reaches chat/AI tooling, only a selection in a real file does.
- `src/cursorContext.ts` — tracks which routine the cursor is currently inside across any open `accessdb:` document (debounced, read-only — never touches `editor.selection`) and persists it to `.accdb-ai/.cursor-context.json`. This is the fallback the `vba-analiza`/`vba-include-tratare-erori` skills use to infer a target routine when the user didn't type a name and there's no active mirror-file selection — covering exactly the breadcrumbs/outline navigation case above, where the cursor moved but nothing got selected.
...
- `src/htmlMockupGenerator.ts` — "Export HTML Mockup": translates the proprietary design headers of Access Forms and Reports into a static visual HTML representation (triggered via context menu). This provides a way to visually inspect the layout of UI components outside of Access.
- `src/pushToAccess.ts` — "Transfera VBE": injects CodeLenses over individual VBA Sub/Function blocks in the mirror files, allowing the user to push only a specific chunk of code directly into the Access VBA Editor without performing a full file save.

**Windows-only, Access 2013 quirks worth knowing**: `SaveAsText` on this environment exports class modules as empty files (no antet/header) — the bridge falls back to reading via the VBE (`viaVbe`) in that case, and such modules must be written back the same way. AccessVBOM ("Trust access to the VBA project object model") being disabled surfaces as `VBE_TRUST_REQUIRED` and requires a manual Trust Center change by the user — never attempt to flip this via the registry. The fire-and-forget post-save compile check (`acCmdCompileAndSaveAllModules`, `fsProvider.ts`'s `scheduleCompileCheck`) re-saves the *whole* VBA project, which can re-touch the object just written beyond what the immediate post-write re-read already captured — `scheduleCompileCheck` re-reads and refreshes that object's cached baseline afterward specifically to avoid a false-positive `CONFLICT` on the next save of the same object.

## Architecture: skills/ (VBA conventions for Access projects)

These are AI Agent skills (Romanian) that operate on VBA source *exported as text* from an Access project (`.bas`/`.form.txt`/`.report.txt`), typically accessed through the AI mirror described above. They encode conventions that are real and dominant across the target application's existing codebase — reuse them, don't invent alternatives:
...
- Both of the above resolve their target routine/module without a typed name too: first an active mirror-file selection (from "Select Code for AI"), then `.accdb-ai/.cursor-context.json` (written by `src/cursorContext.ts` — see above), then the whole open document, before asking for clarification. After applying any edit, both must re-read the file and confirm the change actually landed before reporting success — an edit call that silently failed or was never made must never be reported as applied.
...
- După orice modificare a codului VBA, trebuie rulat skill-ul `vba-progres` pentru a actualiza progresul.
